...
def distributions() -> tuple[Path, Path]:
    ...
    wheels = tuple(directory.glob("meridian_storage_clickhouse-1.0.0-*.whl"))
    sdists = tuple(directory.glob("meridian_storage_clickhouse-1.0.0.tar.gz"))
    assert len(wheels) == 1
    assert len(sdists) == 1
...
def test_wheel_metadata_is_release_ready(distributions: tuple[Path, Path]) -> None:
    ...
    assert "meridian-storage-semantics==1.0.0" in requirements
    assert "meridian-storage-query==1.0.0" in requirements
...
def test_sdist_contains_source_license_notice_and_build_metadata(
    ...
    root = "meridian_storage_clickhouse-1.0.0/"
