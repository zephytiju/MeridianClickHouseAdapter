...
def distributions() -> tuple[Path, Path]:
    ...
    wheels = tuple(directory.glob("meridian_storage_clickhouse-1.1.1-*.whl"))
    sdists = tuple(directory.glob("meridian_storage_clickhouse-1.1.1.tar.gz"))
    assert len(wheels) == 1
    assert len(sdists) == 1
...
def test_sdist_contains_source_license_notice_and_build_metadata(
    ...
    root = "meridian_storage_clickhouse-1.1.1/"
